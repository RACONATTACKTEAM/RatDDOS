...
def proxylist(): # funzione per la creazione della proxylist
	...
	for i in proxiesp:
		filepr.write(i)
	print("Var Olan Ipiler: %s" % (len(open("proxy.txt").readlines())))
...
def main(): # funzione effettiva del programma.
	try:
		...
		proxyget2(url)
		print("Var Olan İpiler: %s" % (len(open("proxy.txt").readlines()))) # rpinta la lunghezza attuale del file, che sarebbe il numero di proxy
